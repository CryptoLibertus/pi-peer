...
function renderTaskHandoffGuidance(envelope) {
  const intent = envelope?.body?.intent || "ask";
  const metadata = envelope?.body?.metadata || {};
  const claimedPaths = metadata.claimedPaths;
  const taskLike = intent === "task" || Boolean(metadata.goalId || metadata.goalClaimId) || (Array.isArray(claimedPaths) && claimedPaths.length > 0);
  if (!taskLike) return "";
...
async function recordGoalProgress(entry, progress, options = {}) {
  ...
  await appendPeerGoalEvent(options.cwd, metadata.goalId, {
    ...
    metadata: {
      ...
      ...(progress.detail !== undefined ? { detail: redactPeerAuditValue(progress.detail, { homeDir: options.homeDir || process.env.HOME || "" }) } : {}),
    },
  });
}
