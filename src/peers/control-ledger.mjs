...
function applyHiveRecord(supervisors, record) {
  ...
  supervisors.set(key || goalId, stripEmpty({
    ...
    maxTicks: positiveNumber(metadata.maxTicks) || current.maxTicks,
    maxPeers: positiveNumber(metadata.maxPeers) || current.maxPeers,
    tickCount: nonNegativeNumber(metadata.tickCount) ?? current.tickCount,
